...
export function CustomIndicatorChart({ points, label, units = '', color = '#8b5cf6' }: Props) {
  const { dark } = useTheme();
  ...
  if (points.length === 0) {
    return (
      ...
      >
        No data — check that all input series have overlapping dates.
      </div>
    );
  }

  const data = points.map((p) => ({ x: p.date, y: p.value }));
  const rgba = color + '1a';
      ...
      options={{
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              label: (ctx) =>
                ` ${(ctx.parsed.y ?? 0).toLocaleString('en-US', { maximumFractionDigits: 4 })}${units ? ' ' + units : ''}`,
            },
          },
        },
        scales: {
